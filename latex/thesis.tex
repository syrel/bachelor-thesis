...
To validate the model we implemented the code editor with an ability of expanding method code right in-place, the Transcript, the Connector and the Documenter. These tools are built on top of the same editor infrastructure around the concept of \textit{adornments} and a single composition tree of graphical elements.

Nonetheless, this work only scratches a surface of what can be done with the Moldable Editor. We already saw that it allows us to implement features and provide new experiences that are simply not possible in other editors. There is still a lot that should be done and we have to walk through a long path that lies in front of us. While the basic infrastructure and underlying model is in place the editor in its current state misses some important for usability features normally used during day-to-day activities such as copy-paste, keyboard shortcuts, multiple text selection and smart cursor navigation. The Moldable Editor does not yet provide an auto-completion mechanism or common decorators which include line numbers, clickable links or code critiques adornments.
